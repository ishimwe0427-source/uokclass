...
|---|--------------------------------|-----------------|
| 1 | `01-public-and-private-ip.png` | Azure Portal → **cloud-uok** → **Overview** (Status Running, Public **20.121.113.179**, Private **172.16.0.4**) |
| 2 | `02-ports-azure-nsg-22-80-443.png` | **Networking** → Inbound rules: SSH 22, HTTP 80, HTTPS 443 → **Allow** |
| 3 | `03-ports-ufw-status.png` | SSH: output of `sudo ufw status` (22, 80, 443 ALLOW) |
| 4 | `04-disk-df-h.png` | SSH: output of `df -h` |
| 5 | `05-network-ip-addr.png` | SSH: output of `ip addr show eth0` (or `ip -4 addr show`) |
| 6 | `06-network-ip-route-and-dns.png` | SSH: `ip route` and `cat /etc/resolv.conf` (one screenshot or two pasted in one image) |
